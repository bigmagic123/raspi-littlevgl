...
#define AUX_BASE            (0x3F000000 + 0x215000) //mini uart
...
static int uart_putc(struct rt_serial_device *serial, char c)
{
    ...
    if (uart->hw_base == AUX_BASE)
    {
        ...
        // while (UART0_FR(uart->hw_base) & (1 << 5)) { }
        // UART0_DR(uart->hw_base) = c;
    }
    else
    {
        while (UART0_FR(uart->hw_base) & (1 << 5)) { }
...
static int uart_getc(struct rt_serial_device *serial)
{
    ...
    {
        if ((UART0_FR(uart->hw_base) & 0x01))
        {
            ch = (UART0_DR(uart->hw_base));
        }
    }
    return ch;
}
